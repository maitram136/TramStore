...
const Books = ({ bookStore, dispatch }) => {
  ...
  const handleDelete = id => {
    dispatch({
      ...
      payload: {
        id,
        callBack: success => {
          if (success) Notification.success('Product Deleted Successfully!');
          else Notification.error('Delete Product Failed!');
        },
      },
    });
  };
  ...
  return (
    <StyledContent>
      ...
      {isModalOpen && (
        <FormModal
          name={editingId ? 'Edit Product' : 'Create a new product'}
